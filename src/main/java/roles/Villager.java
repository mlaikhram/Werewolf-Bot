...
public class Villager extends Role {
    ...
    public Villager(String nickName, User user, WerewolfSession session) {
        super("Villager", nickName, false, user, session);
        this.status = RoleStatus.ALIVE;
        user.openPrivateChannel().queue((channel) -> {
            channel.sendMessage("You are a Villager. You will be trying to execute the werewolves each day").queue();
        });
    }
    ...
    public boolean prompt() {
        if (status == RoleStatus.ALIVE) {
            user.openPrivateChannel().queue((channel) -> {
                channel.sendMessage( session.getRolesPrompt(false) + "You are a Villager. Select the player that you think is a Werewolf").queue();
            });
            return true;
        }
        return false;
