...
public class Lover extends Role {
    ...
    public Lover(String nickName, User user, WerewolfSession session, User partner) {
        ...
        user.openPrivateChannel().queue((channel) -> {
            channel.sendMessage("You are a Lover. You will be trying to execute the werewolves each day. If your partner is killed by the Werewolves, you will die in the same night. Your partner is " + partner.getName()).queue();
        });
    }
    ...
    public boolean prompt() {
        if (status == RoleStatus.ALIVE) {
            user.openPrivateChannel().queue((channel) -> {
                channel.sendMessage( session.getRolesPrompt(false) + "You are a Lover. Select the player that you think is a Werewolf").queue();
            });
            return true;
        }
        return false;
